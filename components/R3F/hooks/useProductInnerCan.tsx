...
const POSITIONS_LAST = POSITIONS[1][2];

const ROTATIONS: VectorArray = [
  [
    [0, 0, 0],
    [0, 0, 0],
    [-0.01, Math.PI * 0.95, Math.PI * 2],
  ],
  [
    [0, Math.PI * 1.95, Math.PI * 2],
    [0, Math.PI * 1.95, Math.PI * 2],
    [0, Math.PI * 1.95, Math.PI * 2],
  ],
];
...
const ROTATIONS_LAST = ROTATIONS[1][2];
...
const animate = (
  ...
  if (positionFrom && positionTo) {
    ...
    const [scalesFrom, scalesTo] = getFromAndToVector(
      SCALES,
      sectionNumber,
      currSectionStep,
    );
    ...
    const scale = calculateVector(scalesFrom, scalesTo, stepPercentage);
...
export default function useProductInnerCan(props: Props) {
