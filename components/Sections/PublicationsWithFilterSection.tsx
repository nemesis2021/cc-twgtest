...
import useGetPublications from '../../utils/hooks/customPostTypes/useGetPublications';
import type { TaxonomyModel } from '../../utils/models/TaxonomyModel';
import type { CategoriesModel } from '../../utils/models/CategoryModel';
import type { PublicationPartialModel } from '../../utils/models/customPostTypes/PublicationModel';
...
type PublicationsState = {
  ...
  hasMore: boolean;
  errorMessage: string;
  publications: PublicationPartialModel[];
};
...
type PublicationsAction =
  ...
  | { type: 'SET_PUBLICATIONS'; payload: PublicationPartialModel[] }
  | { type: 'RESET_FILTERS' };
...
export default function PublicationsWithFilterSection(props: Props) {
  ...
  const hasNextPage =
    publicationsData?.publications.pageInfo.offsetPagination.hasMore;
  const hasPrevPage =
    publicationsData?.publications.pageInfo.offsetPagination.hasPrevious;
  ...
  useEffect(() => {
    ...
    if (!loading) {
      dispatch({
        ...
        payload:
          publicationsData?.publications.edges.map(
            (publication) => publication.node,
          ) || [],
      });
      dispatch({ type: 'SET_VISIBILITY', payload: true });
    }
  }, [publicationsData, loading, isLoadingPage]);

  useEffect(() => {
    if (publicationsData?.publications.pageInfo.offsetPagination.total) {
      const totalPages: number = Math.ceil(
        publicationsData.publications.pageInfo.offsetPagination.total /
          POST_PER_PAGE,
      );
      dispatch({
        type: 'SET_PAGES',
      ...
      <Grid container columnSpacing={CONTENT_GAP} px={{ lg: 5 }}>
        ...
        <Grid item xs={12} md={9}>
          <AnimatedResponsiveContainer>
                    ...
                    {publications && publications.length > 0 ? (
                      publications.map((publication) => (
                        <Grid item xs={12} md={6} key={publication.id}>
                          <PublicationCard data={publication} applyToGrid />
                        </Grid>
                      ))
                    ) : (
                      <Grid
            ...
            {publications.length > 0 && pages.length > 1 && !errorMessage && (
              <Pagination
